...
def get_existing_investment(client, symbol):
    """
    Calculate the current investment in a specific cryptocurrency.
    ...
    Returns:
    float: The current investment in the specified cryptocurrency.
    """
    # Extract the base asset from the symbol (e.g., 'BTC' from 'BTCUSDT')
    base_asset = symbol[:-4]

    # Get the balance of the base asset
    base_asset_balance = get_balance(client, base_asset)

    # Get the current price of the base asset
    current_price = float(client.get_symbol_ticker(symbol=symbol)['price'])

    # Calculate the total investment in the base asset
    total_investment = base_asset_balance * current_price
...
def get_lot_size(client, symbol):
    """Get the lot size step for a symbol."""
    info = client.get_symbol_info(symbol)
    lot_size = float([f['stepSize'] for f in info['filters'] if f['filterType'] == 'LOT_SIZE'][0])
    return lot_size


def format_quantity(quantity, lot_size):
    """Format the quantity according to the lot size."""
    precision = int(round(-math.log10(lot_size), 0))
...
def main_trading_loop(symbol, interval='1m', risk_percentage=5, max_exposure_percentage=15, sleep_time=60):
    while True:
        try:
